...
void SRKSpinTracker::getInitialState(SRKSpinState& stateOut)
{
	stateOut = initialState;
}

void SRKSpinTracker::trackSpin(SRKODEState& theState, double* theField, double timeToTrack, std::vector<SRKODEState>* stepRecord, std::vector<double>* stepTimes)
{
	...
	if(stepRecord != nullptr)
	{
		...
		{
			integrate_adaptive(controlled_stepper, theEquationOfMotion, theState, theState[9], theState[9] + SRKSpinFloat(timeToTrack), SRKSpinFloat(initialStepSize), pushBackStateAndTime(stepRecord, stepTimes));
		}
