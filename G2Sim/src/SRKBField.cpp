...
void SRKBField::addFieldValue(const double localPoint[3], double fieldValue[9])
{
	...
	double Bpp = scalingFactorWUnits; // index is xy, p = positive, n = negative
	double Bpn = scalingFactorWUnits;
	double Bnp = scalingFactorWUnits;
	double Bnn = scalingFactorWUnits;

	double dxByp = Bpp - Bnp;//divide by distance!!
	double dxByn = Bpn - Bnn;
	double dyByp = Bpp - Bpn;
	double dyByn = Bnp - Bnn;

	if(localPoint[0]<0){
		if(localPoint[1]<0){
			fieldValue[g4FieldX] += dxByn*localPoint[1] - dyByn*localPoint[0];
			fieldValue[g4FieldY] += Bnn;
		}
		else{
			fieldValue[g4FieldX] += dxByn*localPoint[1] - dyByp*localPoint[0];
			fieldValue[g4FieldY] += Bnp;
		}
	}
	else{
		if(localPoint[1]<0){
			fieldValue[g4FieldX] += dxByp*localPoint[1] - dyByn*localPoint[0];
			fieldValue[g4FieldY] += Bpn;
		}
		else{
			fieldValue[g4FieldX] += dxByp*localPoint[1] - dyByp*localPoint[0];
			fieldValue[g4FieldY] += Bpp;
		}
	}
