...
public:
	...
	void trackSpin(SRKODEState& theState, double* theField, double timeToTrack, std::vector<SRKODEState>* stepRecord = nullptr, std::vector<double>* stepTimes = nullptr);  //Primary spin tracker
	...
	void getInitialState(SRKSpinState& stateOut); //If manualTracking, then sets state with defaults, otherwise gets it randomly
	inline const TVector3 getInitialMag(){return initialState.M;}
	inline void setInitialMag(const TVector3& inp){ initialState.M=inp;}
	...
	inline void setGlobalField(SRKGlobalField* inp){theEquationOfMotion.setGlobalField(inp); theGlobalField = inp;}
