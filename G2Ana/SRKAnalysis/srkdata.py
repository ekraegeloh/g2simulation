...
def default_file_stats(runtype="g2full"):
...
def write_macro_commands_to_file(macro_file, run_id, srk_settings, run_settings):
	...
	if run_settings['RunType'] == 'deltaOmega':
		...
		macro_file.write('trackSpins ' + str(run_settings['NumTracksPer']))
	else:				#change to different g-2 cases
		macro_file.write('setResultsFilePath ' + srkglobal.results_dir + "Results_RID" + str(run_id) + ".root" + '\n')
		macro_file.write('trackSpins ' + str(run_settings['NumTracks']))
...
def make_macro_from_database(run_id, runtype="nedm"):
	"""Makes a macro file from settings from the database"""
	srk_settings, run_settings = get_settings_from_database(run_id, runtype)
	make_macro(run_id, srk_settings, run_settings)


def make_macro_mult_from_database(run_ids, runtype="nedm"):
...
def get_settings_from_database(run_id, runtype="nedm", db_connection=None):
	...
	srk_settings_out = default_srk_settings(runtype)
